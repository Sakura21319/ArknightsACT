...
{
    public enum CityFacilityKind { Medical, Relay, Power, Overload, PressureVent, RelaySwitch }
    ...
    public sealed class CityFacility25D : MonoBehaviour
    {
        ...
        public float Duration => Kind == CityFacilityKind.Power || Kind == CityFacilityKind.Overload ? 4f : Kind == CityFacilityKind.Relay ? 2.5f : 2f;
        public string Label => Kind switch { CityFacilityKind.Medical => "罗德岛应急补给", CityFacilityKind.Relay => "天灾观测中继", CityFacilityKind.Power => "源石配电柜", CityFacilityKind.Overload => "超载旁路", CityFacilityKind.PressureVent => "紧急泄压阀", _ => "双端接力开关" };
        ...
        public bool CanUse(Health health) => State == CityFacilityState.Ready && health != null && !health.IsDead &&
            (Kind != CityFacilityKind.Medical || health.CurrentHealth < health.MaxHealth - .01f) &&
