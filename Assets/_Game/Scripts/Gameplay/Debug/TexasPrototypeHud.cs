...
namespace ArknightsACT.Gameplay.Debugging
{
    public sealed class TexasPrototypeHud : MonoBehaviour
    {
        ...
        private TexasBuildRuntime _build;

        private void Awake()
        {
            _health = GetComponent<Health>();
            _skillController = GetComponent<PlayerSkillController>();
            _build = GetComponent<TexasBuildRuntime>();
        }
        ...
        private void OnGUI()
        {
            ...
            GUI.Label(new Rect(24, 82, 320, 22), $"[1] Swift Blade: {OnOff(_build?.SwiftBlade ?? false)}");
            GUI.Label(new Rect(24, 102, 320, 22), $"[2] Residual Thunder: {OnOff(_build?.ResidualThunder ?? false)}");
            GUI.Label(new Rect(24, 122, 320, 22), $"[3] Conductive: {OnOff(_build?.Conductive ?? false)}");
        }
